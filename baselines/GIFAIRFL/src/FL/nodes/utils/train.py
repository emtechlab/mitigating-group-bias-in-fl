...
# Training
def update_weights(args, model, device, global_epoch, train_loader, testDataloader, criterion, client_id, r_k, lambda_reg, p):

    optimizer = torch.optim.SGD(model.parameters(), lr=args.lr*(1+(lambda_reg/(p[client_id]))*r_k[client_id]))

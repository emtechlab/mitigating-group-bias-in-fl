...
class LocalBase():
    ...
    def __init__(self, args, device, client_id):
        ...
        if args.do_training == 1:
            ...
            mean_p = aggregate_round_avg_probs(1, output_dir=out_dir)

            # convert to a tensor on the right device
            class_weights = torch.from_numpy(mean_p).float().to(device)
...
def define_localnode(args, device, client_id):
    ...
    if (args.federated_type == 'mwr') | (args.federated_type == 'fedavg') | (args.federated_type == 'fedprox'):

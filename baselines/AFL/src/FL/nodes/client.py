...
class LocalBase():
    ...
    def update_weights(self, client_loaders, test_loader, model, global_epoch):
        ...
        updated_weight, updated_model, avg_grad_norm = update_weights(self.args, model, self.device, global_epoch, client_loaders[self.client_id], test_loader, self.criterion, self.client_id)
...
class Fedavg_Local(LocalBase):
    ...
    def localround(self, client_loaders, test_loader, model, global_epoch, validation_only=False):
        ...
        clients_params = ClientsParams(weight=self.updated_weight, afl_loss=test_loss)           #self.updated_weight 
        
        return clients_params, updated_model, avg_grad_norm

def define_localnode(args, device, client_id):
    
    if (args.federated_type == 'ditto') | (args.federated_type == 'fedavg') | (args.federated_type == 'afl'):

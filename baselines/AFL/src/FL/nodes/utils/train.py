...
def plot_confusion_matrix(args, client_id, cm, classes, epoch ,normalize=True, title='', cmap=plt.cm.Blues):
   ...
   base_name = f"/home/{os.getenv('USER')}/Individual-Subgroup-Fairness/evaluation/{args.federated_type}/{args.dataset}/train_matrix"
...
def update_csv(args, client_idx, test_loss, test_acc, avg_grad_norm):
    ...
    filename = f"/home/{os.getenv('USER')}/Individual-Subgroup-Fairness/evaluation/{args.federated_type}/{args.dataset}/client_train{client_idx}.csv"
...
def update_weights(args, model, device, global_epoch, train_loader, testDataloader, criterion, client_id):
    ...
    for epoch in range(1, args.local_epochs+1):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            batch_grad_norm = torch.sqrt(sum(torch.norm(p.grad.detach())**2 for p in model.parameters() if p.grad is not None))
